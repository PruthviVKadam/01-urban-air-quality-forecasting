...
def _state(cb: CircuitBreaker) -> BreakerState:
    """Helper to read state without mypy narrowing the return type."""
    return cb.state
...
def test_opens_after_threshold() -> None:
    ...
    cb.record_failure()
    assert _state(cb) == BreakerState.open
    assert not cb.allow()
...
def test_half_open_after_recovery_then_success_closes() -> None:
    ...
    assert _state(cb) == BreakerState.open
    clock.advance(10)
    assert _state(cb) == BreakerState.half_open
    assert cb.allow()
    cb.record_success()
    assert _state(cb) == BreakerState.closed
