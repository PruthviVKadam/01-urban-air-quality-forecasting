...
def test_etl_orchestrator(tmp_path: Path) -> None:
    ...
    with patch("app.ingestion.etl.default_data_dir", return_value=tmp_path):
        ...
        mock_registry = {
            "us-nyc-cp": __import__(
                "app.ingestion.station_registry", fromlist=["StationEntry"]
            ).StationEntry(
                ...
                sensors=[
                    __import__(
                        "app.ingestion.station_registry", fromlist=["SensorMapping"]
                    ).SensorMapping(Pollutant.pm25, 3550)
                ],
            )
        }
        ...
        with (
            patch("app.ingestion.etl.get_registry", return_value=mock_registry),
            ...
            bad_m = Measurement(
                ...
                unit="µg/m³",
                source="openaq",
            )  # negative, will be quarantined

            mock_openaq.return_value = (
                FetchResult({}, False, False),
                ParseResult([good_m1, good_m2, bad_m], 0),
            )
