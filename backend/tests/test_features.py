...
def test_build_features(tmp_path: Path) -> None:
    ...
    measurements_store.upsert([
        Measurement(station_id="s1", pollutant=Pollutant.pm25, ts=ts1, value=10.0, unit="µg", source="m"),
        Measurement(station_id="s1", pollutant=Pollutant.o3, ts=ts1, value=20.0, unit="ppb", source="m"),
        Measurement(station_id="s1", pollutant=Pollutant.pm25, ts=ts2, value=15.0, unit="µg", source="m"),
    ])
    
    # Insert weather
    weather_store.upsert([
        WeatherObservation(station_id="s1", ts=ts1, temperature_c=15.0, humidity_pct=50.0, wind_speed_ms=2.0, pressure_hpa=1010.0),
        WeatherObservation(station_id="s1", ts=ts2, temperature_c=16.0, humidity_pct=48.0, wind_speed_ms=2.5, pressure_hpa=1009.0)
    ])
    ...
    con = duckdb.connect()
    try:
        df = con.execute(f"SELECT * FROM read_parquet('{features_path.as_posix()}') ORDER BY ts").df()
