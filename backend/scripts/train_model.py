...
def train_and_evaluate(df: pd.DataFrame, target_col: str) -> Tuple[HistGradientBoostingRegressor, Dict, Dict]:
    ...
    feature_cols = [
        ...
        f'{target_col}_lag_1h', f'{target_col}_lag_24h', f'{target_col}_roll_mean_24h',
    ...
    if len(unique_times) < 10:
        logger.warning("Not enough data to train properly. Will train anyway for pipeline verification.")
    ...
    y_test = df_test['future_target']
    
    if len(X_train) < 20:
        logger.warning(f"Training set for {target_col} is very small ({len(X_train)} rows). Skipping training.")
        return HistGradientBoostingRegressor(), {}, {}
