...
def add_persistence_baseline(df: pd.DataFrame, target_col: str, horizons: List[int]) -> pd.DataFrame:
...
def add_seasonal_baseline(df: pd.DataFrame, target_col: str) -> pd.DataFrame:
    ...
    df[f'{target_col}_seasonal_avg'] = df.groupby(['station_id', 'day_of_week', 'hour_of_day'])[target_col].transform(
