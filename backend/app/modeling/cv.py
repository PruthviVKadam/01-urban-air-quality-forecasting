    ...
    for target in target_cols:
        ...
        for h in horizons:
            ...
            if persist_mask.sum() > 0:
                ...
                results[target]["persistence"]["mae"].append(
                    mean_absolute_error(df_p[f'true_{h}h'], df_p[f'{target}_persist_{h}h'])
                )
                results[target]["persistence"]["rmse"].append(
            ...
            if season_mask.sum() > 0:
                ...
                results[target]["seasonal"]["mae"].append(
                    mean_absolute_error(df_s[f'true_{h}h'], df_s[f'seasonal_pred_{h}h'])
                )
                results[target]["seasonal"]["rmse"].append(
                    root_mean_squared_error(df_s[f'true_{h}h'], df_s[f'seasonal_pred_{h}h'])
                )
        
        # Average across horizons
        for model in ["persistence", "seasonal"]:
            # Type ignore because we are replacing a list with a float, which is fine for the return type
            if results[target][model]["mae"]:
                results[target][model]["mae"] = float(np.mean(results[target][model]["mae"])) # type: ignore
                results[target][model]["rmse"] = float(np.mean(results[target][model]["rmse"])) # type: ignore
            else:
                results[target][model]["mae"] = 0.0 # type: ignore
                results[target][model]["rmse"] = 0.0 # type: ignore
